...
export default {
  ...
  methods: {
    ...
    // },
    ExpandNodeInfo () {
        this.$emit("foreInfo", this.responseData)
    },
    changeKDLoading () {
        this.$emit("foreChange", false)
    },
    getNodeRelation (id) {
      axios
        .post('http://106.14.239.166/graph/api/graph/expandNode/', {id: id})
        .then(response => {
          ...
          this.changeKDLoading()
        })
        .catch(error => console.log(error))
    },
    getNodeRelationWithId (responseData) {
        console.log(responseData)
        this.nodes = []
        this.edges = []
        responseData.relations.forEach((relation) => {
            let edge = {}
            edge.target = relation.end_id
            edge.source = relation.start_id
            edge.id = relation.id
        ...
        responseData.nodes.forEach((relatedNode) => {
            let node = {}
            node.id = relatedNode.id
            node.name = relatedNode.name
            node.labels = relatedNode.labels
